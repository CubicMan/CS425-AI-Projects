...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:  # jail edge case if ghost is captured
            jailPosition = self.getJailPosition() #gets the jail position
            allPossible[jailPosition] = 1.0 #sets the counter to 1

        elif noisyDistance != None:
            for p in self.legalPositions: #position = for every possible ghost position
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                #manhattan distance from pacman to ghost position
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        newBeliefs = util.Counter() #counter object to store the new belief distribution
        newPosDist = util.Counter() #counter object to store the new position distribution

        for oldPos in self.legalPositions: #for all possible old ghost positions
            #uses the given newPosDict line of code
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))

            #for all items in the newPosDist counter
            for newPos, prob in newPosDist.items():
                #compute multiple position distributions for a single update by adding up previous probabilities
                if newPosDist[newPos] > 0: newBeliefs[newPos] = newBeliefs[newPos] + (prob * self.beliefs[oldPos])
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        count = 0 #stores the number of particles currently in the self.particles list
        particleCount = self.numParticles #stores the numbers of particles
        self.particles = [] #self.particles list

        for i in range(particleCount): #for the number of particles
            index = i % len(self.legalPositions) #gets the index of the position from possible positions
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()
        uniqueParticles = util.Counter()

        #belief distribution based on number of particles
        newList = []
        if noisyDistance == None:
            ghostPosition = self.getJailPosition() #stores the ghost position
            allPossible[ghostPosition] = 1.0 #sets it to 1.0
            for item in self.particles:
                item = ghostPosition
            self.beliefs = allPossible

        elif noisyDistance != None:
            # make list of unique particles
            for particle in self.particles:
                uniqueParticles[particle] += 1

            for p in self.particles: # for every particle
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    allPossible[p] = emissionModel[trueDistance] * uniqueParticles[p]

        # when all particles receive 0 weight, they will be re-created with initializeUniformly.
        if allPossible.totalCount() == 0:
            self.initializeUniformly(gameState)

        elif allPossible.totalCount() != 0:
            particleList = []
            particleNumber = self.numParticles
            for i in range(particleNumber):
                #sample allPossible counter based on the particle number
                particleList.append(util.sample(allPossible))
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles: #for every particle
            # converts to a distribution by adding one to the counter
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        numGhosts = self.numGhosts  # number of ghosts
        combinations = itertools.product(self.legalPositions, repeat = numGhosts) #based on Ghost Number
        tupleList = list(combinations) #particles are stored in a list
        random.shuffle(tupleList) #shuffle list

        count = 0 #stores the number of particles currently in the self.particles list
        self.particles = [] #particle list

        for particle in tupleList:
                if count < self.numParticles:
                    # add particles to self.particles if number is less than the particle number
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        allPossible = util.Counter()
        for particle in self.particles:
            emissionProbability = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    #set particles to the ghostinJail if noisyDistances is None
                    particle = self.getParticleWithGhostInJail(particle, i)

                else:
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
                    emissionProbability = emissionProbability * emissionModels[i][trueDistance]
            allPossible[particle] = allPossible[particle] + emissionProbability
        ...
        if allPossible.totalCount() == 0:
            ...
            particleNumber = self.numParticles
            for i in range(particleNumber):
                particleList.append(util.sample(allPossible)) #sample allPossible for the number of particles
            self.particles = particleList
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts): #To loop over the ghosts
                    newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                                 i, self.ghostAgents[i])
                    newParticle[i] = util.sample(newPosDist) #sample the new Distribution
            "*** END YOUR CODE HERE ***"
